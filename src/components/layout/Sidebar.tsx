...
const navItems = [
  ...
  { to: "/calendar", label: "Timeline", icon: FiCalendar },
  { to: "/settings", label: "Settings", icon: FiSettings },
];

const pinned = ["Pending orders", "Out of stock", "Top products"];
  ...
  return (
    ...
    >
      <div className="mb-6 flex items-center gap-3">
        ...
        {!collapsed && (
          <div>
            <p className="text-xs uppercase tracking-[0.28em] text-slate-500">WooCommerce</p>
            <p className="text-lg font-semibold text-slate-900">Nova Commerce OS</p>
          </div>
        )}
      </div>
      ...
      {!collapsed && (
        <div className="mb-5 rounded-2xl bg-white/70 p-3">
          ...
          <p className="text-sm font-semibold text-slate-800">Store Operations</p>
          <p className="text-xs text-slate-500">Live WooCommerce Data</p>
        </div>
      )}
      ...
      <nav className="flex flex-1 flex-col gap-1.5">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {!collapsed && active && (
                <motion.span layoutId="activeIndicator" className="absolute inset-y-1 left-1 w-1 rounded-full bg-white/70" />
              )}
        ...
        {!collapsed && (
          <div className="mt-3 space-y-2">
            ...
            {pinned.map((item) => (
              <button key={item} className="flex w-full items-center gap-2 rounded-xl px-2 py-2 text-left text-xs text-slate-600 transition hover:bg-white/70">
      ...
      <div className="glass soft-card mt-4 rounded-2xl p-3">
        <div className="flex items-center gap-3">
          <img src="https://i.pravatar.cc/80?img=21" alt="profile" className="h-10 w-10 rounded-full" />
          {!collapsed && (
            <>
              <div>
                <p className="text-sm font-semibold text-slate-800">Sophia Lin</p>
                <p className="text-xs text-slate-500">Store Director</p>
              </div>
              <button onClick={logout} className="ml-auto rounded-xl p-2 text-slate-500 hover:bg-white/80">
                <FiLogOut />
              </button>
            </>
          )}
